...
import { User, MoonStar, ExternalLink, Copy, Check, Menu, X } from 'lucide-react';
  ...
  return (
    <div className="flex h-screen bg-[#081423] text-white">
      ...
      <div className="flex-1 flex flex-col min-w-0">
          ...
          {shouldShowSuggestions ? (
            ...
            <div className="max-w-4xl mx-auto p-4 sm:p-6 md:p-8">
              <div className="space-y-6 sm:space-y-8">
                {messages.map((msg) => (
                  <div key={msg.id} className={`flex items-start gap-3 sm:gap-4 ${msg.isUser ? 'flex-row-reverse' : ''}`}>
                    ...
                    <div className={`group relative rounded-xl sm:rounded-2xl p-4 sm:p-6 max-w-[90%] sm:max-w-3xl ${
                      msg.isUser 
                      ...
                      {msg.isUser ? (
                        <div className="whitespace-pre-wrap leading-relaxed text-sm sm:text-base">
                          {msg.attachment && (
                            <div className="mb-2">
                              <img src={`data:${msg.attachment.type};base64,${msg.attachment.data}`} alt={msg.attachment.name} className="max-w-full sm:max-w-xs rounded-lg" />
                            </div>
                          )}
                          <p>{msg.text}</p>
                        </div>
                ...
                {isLoading && (
                  <div className="flex items-start gap-3 sm:gap-4">
                    <div className="flex-shrink-0 h-8 w-8 sm:h-10 sm:w-10 rounded-full bg-cyan-500 flex items-center justify-center">
                      <MoonStar className="h-5 w-5 text-white" />
                    </div>
                    <div className="rounded-xl sm:rounded-2xl p-4 sm:p-6 max-w-lg bg-slate-800 border border-slate-700">
                      <div className="flex items-center space-x-2">
                        ...
                        <span className="text-xs sm:text-sm text-slate-400 ml-2">Thinking...</span>
                      </div>
                    </div>
                  </div>
                )}
        ...
        <footer className="bg-[#081423] p-2 sm:p-4 md:p-6">
      ...
      {showAlert && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center p-4">
          ...
          />
          <div className="relative bg-slate-800 border border-slate-700 rounded-xl p-4 sm:p-6 max-w-lg w-full shadow-2xl mx-4 max-h-[90vh] overflow-y-auto">
            {/* close button for mobile */}
            <button
              onClick={handleAlertClose}
              className="absolute top-3 right-3 p-2 rounded-lg text-slate-400 hover:text-white hover:bg-slate-700 transition-colors md:hidden"
            ...
            <div className="space-y-3 text-slate-300">
              ...
              <p className="text-xs text-center text-slate-400 pt-2">
                If you won&#39;t be using Indonesian, you can hide this notification.
              </p>
            </div>
